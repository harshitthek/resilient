"""
DeepSeek Agent Adapter for Resilient Benchmark Pipeline.

Supports models: 'deepseek-coder', 'deepseek-chat'.
"""
...
class DeepSeekAgent(AgentAdapter):
    def __init__(self, model_id: str = "deepseek-coder", api_key: Optional[str] = None):
    ...
    def dispatch(self, ctx: RepoContext) -> RunResult:
        """Synchronous dispatch using DeepSeek API (OpenAI-compatible base_url)."""
        if not self.api_key:
            return RunResult(status="failed", error="DEEPSEEK_API_KEY environment variable not set")

        work_dir = tempfile.mkdtemp(prefix=f"resilient_deepseek_{ctx.issue_number}_")
        try:
            ...
            # 2. Call DeepSeek OpenAI-compatible API
            try:
                import openai
                ...
                return RunResult(status="failed", error=f"DeepSeek API error: {str(exc)[:300]}")

        finally:
            shutil.rmtree(work_dir, ignore_errors=True)

    def poll(self, session_id: str) -> RunResult:
        return RunResult(status="failed", error="poll() called on sync DeepSeek agent")
