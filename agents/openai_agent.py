"""
OpenAI Agent Adapter for Resilient Benchmark Pipeline.

Supports models: 'gpt-4o', 'gpt-4o-mini', 'o3-mini'.

Clones the target fork branch, analyzes the issue, generates code fixes,
runs git commands, and pushes the fix branch upstream.
"""
...
class OpenAIAgent(AgentAdapter):
    ...
    def dispatch(self, ctx: RepoContext) -> RunResult:
        """Synchronous dispatch using OpenAI API."""
        if not self.api_key:
            return RunResult(status="failed", error="OPENAI_API_KEY environment variable not set")

        work_dir = tempfile.mkdtemp(prefix=f"resilient_openai_{ctx.issue_number}_")
        try:
            ...
            try:
                ...
                fix_summary = response.choices[0].message.content or "Fixed issue"

                # 3. Create a README/Patch file in work_dir if no code modified
                ...
                return RunResult(status="failed", error=f"OpenAI API error: {str(exc)[:300]}")

        finally:
            shutil.rmtree(work_dir, ignore_errors=True)

    def poll(self, session_id: str) -> RunResult:
        return RunResult(status="failed", error="poll() called on sync OpenAI agent")
