"""
SambaNova Systems Agent Adapter for Resilient Benchmark Pipeline.

High-Speed Cloud Inference Tier via SambaNova API.
Supports models: 'Meta-Llama-3.3-70B-Instruct', 'DeepSeek-R1'.
"""
...
class SambaNovaAgent(AgentAdapter):
    def __init__(self, model_id: str = "Meta-Llama-3.3-70B-Instruct", api_key: Optional[str] = None):
        self.model_id = model_id
        self.api_key = api_key or os.environ.get("SAMBANOVA_API_KEY", "")

    @property
    def name(self) -> str:
        return f"sambanova/{self.model_id}"
    ...
    def dispatch(self, ctx: RepoContext) -> RunResult:
        """Synchronous dispatch using SambaNova Cloud API."""
        if not self.api_key:
            return RunResult(status="failed", error="SAMBANOVA_API_KEY environment variable not set")

        work_dir = tempfile.mkdtemp(prefix=f"resilient_sambanova_{ctx.issue_number}_")
        try:
            ...
            # 2. Call SambaNova Cloud API endpoint (OpenAI-compatible)
            prompt = (
            ...
            try:
                ...
                with open(fix_file, "w", encoding="utf-8") as f:
                    f.write(f"# Resilient SambaNova High-Speed AI Fix Summary ({self.model_id})\n\n{fix_summary}\n")
                ...
                return RunResult(status="failed", error=f"SambaNova API error: {str(exc)[:300]}")

        finally:
            shutil.rmtree(work_dir, ignore_errors=True)

    def poll(self, session_id: str) -> RunResult:
        return RunResult(status="failed", error="poll() called on sync SambaNova agent")
